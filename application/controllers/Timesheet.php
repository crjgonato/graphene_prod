...
class Timesheet extends MY_Controller {
	...
	public function import_attendance() {
	 ...
	 public function task_details() {
		...
		/* get User info*/
		$u_created = $this->Graphene_model->read_user_info($result[0]->created_by);
	...
	public function update_task_status() {
          ...
          foreach($task->result() as $r) {
			...
			if($r->assigned_to == '' || $r->assigned_to == 'None') {
				...
				foreach(explode(',',$r->assigned_to) as $uid) {
					$assigned_to = $this->Graphene_model->read_user_info($uid);
			...
			//$ol = 'A';
			/* get User info*/
			$u_created = $this->Graphene_model->read_user_info($r->created_by);
			$f_name = $u_created[0]->first_name.' '.$u_created[0]->last_name;
			...
			// task end date
			$tdate = $this->Graphene_model->set_date_format($r->end_date);
	 ...
	 public function comments_list()
     {
		...
		$id = $this->uri->segment(3);
		$session = $this->session->userdata('username');
		$ses_user = $this->Graphene_model->read_user_info($session['user_id']);
		if(!empty($session)){ 
		...
		// get user > employee_
		$employee = $this->Graphene_model->read_user_info($r->user_id);
		// employee full name
		...
		$date = $this->Graphene_model->set_date_format($_date[0]);
		// profile picture
	...
	public function set_comment() {
		...
		if($this->input->post('graphene_comment')==='') {
       		 $Return['error'] = "The comment field is required.";
		} 
		$graphene_comment = $this->input->post('graphene_comment');
		$qt_graphene_comment = htmlspecialchars(addslashes($graphene_comment), ENT_QUOTES);
		...
		$data = array(
		'task_comments' => $qt_graphene_comment,
  ...
  public function attendance_list() {
		...
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
		// $remarks = $this->Employee_model->remarks();
	 ...
	 // date attendance list > timesheet
    public function date_wise_list() {
		...
		$employee = $this->Graphene_model->read_user_info($employee_id);
       ...
       // foreach($employee->result() as $r) {
		...
		// set date format >
		$tdate = $this->Graphene_model->set_date_format($attendance_date);
	 ...
	 public function update_attendance_list() {
		...
		/*// get user info >
		$user = $this->graphene_model->read_user_info($employee_id);
		// user full name
		...
		$data = array();
          ...
          foreach($attendance_employee->result() as $r) {
			...
			$cidate = $this->Graphene_model->set_date_format($att_date_in[0]);
			...
			if($r->clock_out=='') {
				...
				$codate = $this->Graphene_model->set_date_format($att_date_out[0]);
		...
		if ($result == TRUE) {
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				foreach($this->input->post('assigned_to') as $p_employee) {
					...
					// assigned by
					$user_info = $this->Graphene_model->read_user_info($this->input->post('user_id'));	
					$full_name = $user_info[0]->first_name.' '.$user_info[0]->last_name;		
					
					// assigned to
					$user_to = $this->Graphene_model->read_user_info($p_employee);	
					//get company info
					$cinfo = $this->Graphene_model->read_company_setting_info(1);
					//get email template
					$template = $this->Graphene_model->read_email_template(14);
	...
	public function add_leave() {
		...
		$withpay = $this->input->post('withpay');
		...
		if ($result == TRUE) {
			$Return['result'] = 'Leave added Successfully';
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				$cinfo = $this->Graphene_model->read_company_setting_info(1);
				//get email template
				$template = $this->Graphene_model->read_email_template(5);
				//get employee info
	...
	public function update_leave_status() {
		...
		if($setting[0]->enable_email_notification == 'yes') {
			...
			if($this->input->post('status') == 2){
				...
				$cinfo = $this->Graphene_model->read_company_setting_info(1);
				//get email template
				$template = $this->Graphene_model->read_email_template(6);
				//get employee info
				$user_info = $this->Graphene_model->read_user_info($timesheet[0]->employee_id);
				
				$full_name = $user_info[0]->first_name.' '.$user_info[0]->last_name;
				
				$from_date = $this->Graphene_model->set_date_format($timesheet[0]->from_date);
				$to_date = $this->Graphene_model->set_date_format($timesheet[0]->to_date);
			...
			<img src="'.$logo.'" title="'.$cinfo[0]->company_name.'"><br>'.str_replace(array("{var site_name}","{var site_url}","{var leave_start_date}","{var leave_end_date}"),array($cinfo[0]->company_name,site_url(),$from_date,$to_date),htmlspecialchars_decode(stripslashes($template[0]->message))).'</div>';
				...
				$cinfo = $this->Graphene_model->read_company_setting_info(1);
				//get email template
				$template = $this->Graphene_model->read_email_template(7);
				//get employee info
				$user_info = $this->Graphene_model->read_user_info($timesheet[0]->employee_id);
	...
	public function read()
	{
		...
		//remarks =  $this->Timesheet_model->remarks();
