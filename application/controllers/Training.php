    ...
    public function training_list()
     {
		...
		// get start date
		$start_date = $this->Graphene_model->set_date_format($r->start_date);
		// get end date
		$finish_date = $this->Graphene_model->set_date_format($r->finish_date);
		// training date
		$training_date = $start_date.' to '.$finish_date;
		// set currency
		$training_cost = $this->Graphene_model->currency_sign($r->training_cost);
		/* get Employee info*/
		if($r->employee_id == '') {
			$ol = '--';
		} else {
			...
			foreach(explode(',',$r->employee_id) as $uid) {
				$user = $this->Graphene_model->read_user_info($uid);
		...
		if ($result == TRUE) {
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				foreach($this->input->post('employee_id') as $p_employee) {
					...
					//get email template
					$template = $this->Graphene_model->read_email_template(13);
