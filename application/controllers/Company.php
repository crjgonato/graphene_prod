...
class Company extends MY_Controller {
    ...
    public function company_list()
     {
		...
		$data = array();
          ...
          foreach($company->result() as $r) {
			  ...
			  // get country
			  $country = $this->Graphene_model->read_country_info($r->country);
			  // get user
	...
	public function add_company() {
		...
		if($name==='') {
			$Return['error'] = $this->lang->line('graphene_error_name_field');
		} else if( $this->input->post('company_type')==='') {
			$Return['error'] = $this->lang->line('graphene_error_ctype_field');
		} else if($contact_number==='') {
			$Return['error'] = $this->lang->line('graphene_error_contact_field');
		} else if($email==='') {
			$Return['error'] = $this->lang->line('graphene_error_cemail_field');
		} else if($website==='') {
			$Return['error'] = $this->lang->line('graphene_error_website_field');
		}  else if($city==='') {
			$Return['error'] = $this->lang->line('graphene_error_city_field');
		} else if($zipcode==='') {
			$Return['error'] = $this->lang->line('graphene_error_zipcode_field');
		} else if($country==='') {
		...
		/* Check if file uploaded..*/
			...
			$fname = 'no file';
			 $Return['error'] = $this->lang->line('graphene_error_logo_field');
		} else {
			if(is_uploaded_file($_FILES['logo']['tmp_name'])) {
				...
				if(in_array($ext,$allowed)){
					...
					$Return['error'] = $this->lang->line('graphene_error_attatchment_type');
				}
			}
		}
		...
		'government_tax' => $this->input->post('graphene_gtax'),
		'trading_name' => $this->input->post('trading_name'),
		'registration_no' => $this->input->post('registration_no'),
		'email' => $this->input->post('email'),
		'contact_number' => $this->input->post('contact_number'),
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_success_add_company');
		} else {
	...
	public function update() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_success_update_company');
		} else {
			$Return['error'] = $Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
		exit;
		}
	...
	public function delete() {
		...
		if(isset($id)) {
			$Return['result'] = $this->lang->line('graphene_success_delete_company');
		} else {
			$Return['error'] = $Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
