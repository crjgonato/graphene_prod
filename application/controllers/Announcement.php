...
class Announcement extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['breadcrumbs'] = $this->lang->line('announcements');
    ...
    public function announcement_list()
     {
		...
		$data[] = array(
			'<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_edit').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-announcement_id="'. $r->announcement_id . '"><i class="fa fa-pencil-square-o"></i></button></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_view').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-announcement_id="'. $r->announcement_id . '"><i class="fa fa-eye"></i></button></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_delete').'"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->announcement_id . '"><i class="fa fa-trash-o"></i></button></span>',
			$r->title,
			$r->summary,
	...
	public function add_announcement() {
		...
		if($this->input->post('title')==='') {
       		$Return['error'] = $this->lang->line('graphene_error_title');
		} else if($this->input->post('start_date')==='') {
			$Return['error'] = $this->lang->line('graphene_error_start_date');
		} else if($this->input->post('end_date')==='') {
			$Return['error'] = $this->lang->line('graphene_error_end_date');
		} else if($st_date > $ed_date) {
			$Return['error'] = $this->lang->line('graphene_error_start_end_date');
       		...
       		$Return['error'] = $this->lang->line('graphene_error_summary_field');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_success_add_announcement');
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				/*foreach($this->input->post('designation_id') as $designation_id) {
					...
					$user_info = $this->Graphene_model->read_user_info_bydesignation($designation_id);				
						get company info
					$cinfo = $this->Graphene_model->read_company_setting_info(1);
					get email template
					$template = $this->Graphene_model->read_email_template(4);
	...
	public function update() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_success_update_announcement');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function delete() {
		...
		if(isset($id)) {
			$Return['result'] = $this->lang->line('graphene_success_delete_announcement');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
