...
class Employees extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index() {
			...
			$data['breadcrumbs'] = $this->lang->line('employees');
	...
	public function directory() {
			...
			$data['breadcrumbs'] = $this->lang->line('employees_directory');
    ...
    public function employees_list() {
        ...
        foreach($employee->result() as $r) {
			...
			// user full name
			$full_name = $r->last_name.', '.$r->first_name.' '.$r->middle_name;	
			//
			//$online
			//$profile_background;
			// get status
			if($r->is_active==0): $status = 'Inactive';
			elseif($r->is_active==1): $status = 'Active'; endif;
			//$online = '<i class="status bg-success bottom right"></i>'
			// <span class="dotgreen bg-success">&nbsp;</span>
			
			if($r->online==0): $r->online = '<span class="dotred bg-danger" title="Offline">&nbsp;</span>';
			elseif($r->online==1): $r->online = '<span class="dotgreen bg-success" title="Online">&nbsp;</span>'; endif;
			// user role
			...
			if($r->user_role_id != '1' && $r->user_id!= '1') {
				$option = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_view_details').'"><a href="employees/detail/'.$r->user_id.'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" title="View Details"><i class="fa fa-arrow-circle-right"></i></button></a></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_delete').'"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->user_id . '"><i class="fa fa-trash-o"></i></button></span>';
			} else {
				$option = '';
			}
	  ...
	  public function detail() {
			...
			$role_resources_ids = $this->Graphene_model->user_role_resource();
			$data['breadcrumbs'] = $this->lang->line('graphene_employee_details');
			...
			$data = array(
				'breadcrumbs' => $this->lang->line('graphene_employee_detail'),
				...
				'telephone_no' => $result[0]->telephone_no,
				'religions' => $result[0]->religions, // new Added Employee Religion
				'nationality' => $result[0]->nationality,   // new Added Employee Nationality
				'age' => $result[0]->age,   // new Added Employee age
				'height' => $result[0]->height, 
				'weight' => $result[0]->weight, 
				'bloodtype' => $result[0]->bloodtype, 
				'complexion' => $result[0]->complexion, 
				'hairdescription' => $result[0]->hairdescription, 
				'scar_marks' => $result[0]->scar_marks, 
				...
				'all_countries' => $this->Graphene_model->get_countries(),
				...
				'philh_no' => $result[0]->philh_no,
				'pagibig_no' => $result[0]->pagibig_no,
				'organization' => $result[0]->organization,
				'orgmem_date' => $result[0]->orgmem_date
				);
	...
	public function add_employee() {
		...
		if($this->input->post('first_name')==='') {
       	$Return['error'] = $this->lang->line('graphene_employee_error_first_name');
		} else if($this->input->post('middle_name')==='') {
			$Return['error'] = $this->lang->line('graphene_employee_error_middle_name');
		} else if($this->input->post('last_name')==='') {
			$Return['error'] = $this->lang->line('graphene_employee_error_last_name');
		}else if($this->input->post('employee_id')==='') {
			 $Return['error'] = $this->lang->line('graphene_employee_error_employee_id');
		} else if(empty($this->input->post('date_of_joining'))) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_joining_date');
		} else if(empty($this->input->post('department_id'))) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_department');
		} else if(empty($this->input->post('designation_id'))) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_designation');
		} else if(empty($this->input->post('role'))) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_user_role');
		} else if(empty($this->input->post('username'))) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_username');
		} else if(empty($this->input->post('email'))) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_email');
		} else if (!filter_var($this->input->post('email'), FILTER_VALIDATE_EMAIL)) {
			$Return['error'] = $this->lang->line('graphene_employee_error_invalid_email');
		} else if(empty($this->input->post('contact_no'))) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_contact_number');
		} else if(empty($this->input->post('telephone_no'))) {
				$Return['error'] = $this->lang->line('graphene_employee_error_telephone_number');
		} else if(empty($this->input->post('password'))) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_password');
		} else if(strlen($this->input->post('password')) < 6) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_password_least');
		} else if($this->input->post('password')!==$this->input->post('confirm_password')) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_password_not_match');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_success_add_employee');
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				//get email template
				$template = $this->Graphene_model->read_email_template(8);
	...
	public function basic_info() {
		...
		'hairdescription' => $this->input->post('hairdescription'),
		'scar_marks' => $this->input->post('scar_marks'),
		'address' => $this->input->post('address'),
		'tin_no' => $this->input->post('tin_no'),
		'sss_no' => $this->input->post('sss_no'),
		'philh_no' => $this->input->post('philh_no'),
		'pagibig_no' => $this->input->post('pagibig_no'),
		'organization' => $this->input->post('organization'),
		'orgmem_date' => $this->input->post('orgmem_date')
		
		);
		$id = $this->input->post('user_id');
		$result = $this->Employees_model->basic_info($data,$id);
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_basic_info_updated');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function profile_picture() {
		...
		if($_FILES['p_file']['size'] == 0 && null ==$this->input->post('remove_profile_picture')) {
			$Return['error'] = $this->lang->line('graphene_employee_select_picture');
		} else {
			if(is_uploaded_file($_FILES['p_file']['tmp_name'])) {
				...
				if(in_array($ext,$allowed)){
					...
					if ($result == TRUE) {
						$Return['result'] = $this->lang->line('graphene_employee_picture_updated');
					...
					$Return['error'] = $this->lang->line('graphene_employee_picture_type');
				}
				}
			}
	...
	public function social_info() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_success_update_social_info');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function update_contacts_info() {
		...
		if($this->input->post('salutation')==='') {
       		 $Return['error'] = $this->lang->line('graphene_employee_error_salutation');
		} else if($this->input->post('contact_name')==='') {
			$Return['error'] = $this->lang->line('graphene_employee_error_contact_name');
		} else if($this->input->post('relation')==='') {
			 $Return['error'] = $this->lang->line('graphene_employee_error_grp');
		} else if($this->input->post('primary_email')==='') {
			 $Return['error'] = $this->lang->line('graphene_employee_error_pemail');
		} else if($this->input->post('mobile_phone')==='') {
			 $Return['error'] = $this->lang->line('graphene_employee_error_mobile');
		} else if($this->input->post('city')==='') {
			 $Return['error'] = $this->lang->line('graphene_error_city_field');
		} else if($this->input->post('country')==='') {
			 $Return['error'] = $this->lang->line('graphene_error_country_field');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_contact_info_updated');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function document_info() {
		...
		if($this->input->post('document_type_id')==='') {
       		 $Return['error'] = $this->lang->line('graphene_employee_error_d_type');
		} else if($this->input->post('date_of_expiry')==='') {
			$Return['error'] = $this->lang->line('graphene_employee_error_expiry_date');
		} else if($this->input->post('title')==='') {
			 $Return['error'] = $this->lang->line('graphene_employee_error_document_title');
		} else if($this->input->post('email')==='') {
			 $Return['error'] = $this->lang->line('graphene_error_notify_email_field');
		}
		
		/* Check if file uploaded..*/
		else if($_FILES['document_file']['size'] == 0) {
			$Return['error'] = $this->lang->line('graphene_employee_select_d_file');
		} else {
			if(is_uploaded_file($_FILES['document_file']['tmp_name'])) {
				...
				if(in_array($ext,$allowed)){
					...
					$Return['error'] = $this->lang->line('graphene_employee_document_file_type');
				}
			}
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_d_info_added');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function immigration_info() {
		...
		if($this->input->post('document_type_id')==='') {
			...
			$Return['error'] = $this->lang->line('graphene_employee_error_d_number');
		} else if($this->input->post('issue_date')==='') {
			$Return['error'] = $this->lang->line('graphene_employee_error_d_issue');
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_img_info_added');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function e_immigration_info() {
		...
		/* Check if file uploaded..*/
			...
			$data = array(
				...
				if ($result == TRUE) {
					$Return['result'] = $this->lang->line('graphene_employee_img_info_updated');
				} else {
					$Return['error'] = $this->lang->line('graphene_error_msg');
				}
				$this->output($Return);
			...
			if(is_uploaded_file($_FILES['document_file']['tmp_name'])) {
				...
				if(in_array($ext,$allowed)){
					...
					$result = $this->Employees_model->img_document_info_update($data,$e_field_id);
					if ($result == TRUE) {
						$Return['result'] = $this->lang->line('graphene_employee_d_info_updated');
					} else {
						$Return['error'] = $this->lang->line('graphene_error_msg');
					}
					$this->output($Return);
	...
	public function qualification_info() {
		...
		if($this->input->post('name')==='') {
       		 $Return['error'] = $this->lang->line('graphene_employee_error_sch_uni');
		} else if($this->input->post('from_year')==='') {
			$Return['error'] = $this->lang->line('graphene_employee_error_frm_date');
		} else if($this->input->post('to_year')==='') {
			 $Return['error'] = $this->lang->line('graphene_employee_error_to_date');
		} else if($st_date > $ed_date) {
			$Return['error'] = $this->lang->line('graphene_employee_error_date_shouldbe');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_error_q_info_added');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function e_qualification_info() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_error_q_info_updated');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function work_experience_info() {
		...
		if($this->input->post('company_name')==='') {
       		 $Return['error'] = $this->lang->line('graphene_employee_error_company_name');
			...
			$Return['error'] = $this->lang->line('graphene_employee_error_frm_date');
			 ...
			 $Return['error'] = $this->lang->line('graphene_employee_error_post');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_error_w_exp_added');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function e_work_experience_info() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_error_w_exp_updated');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function bank_account_info() {
		...
		if($this->input->post('account_title')==='') {
       		 $Return['error'] = $this->lang->line('graphene_employee_error_acc_title');
		} else if($this->input->post('account_number')==='') {
			$Return['error'] = $this->lang->line('graphene_employee_error_acc_number');
		} else if($this->input->post('bank_name')==='') {
			 $Return['error'] = $this->lang->line('graphene_employee_error_bank_name');
		} else if($this->input->post('bank_code')==='') {
			 $Return['error'] = $this->lang->line('graphene_employee_error_bank_code');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_error_bank_info_added');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function e_bank_account_info() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_error_bank_info_updated');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function contract_info() {
		...
		if($this->input->post('contract_type_id')==='') {
       		 $Return['error'] = $this->lang->line('graphene_employee_error_contract_type');
		} else if($this->input->post('title')==='') {
       		 $Return['error'] = $this->lang->line('graphene_employee_error_contract_title');
		} else if($this->input->post('from_date')==='') {
			$Return['error'] = $this->lang->line('graphene_employee_error_frm_date');
		} else if($this->input->post('to_date')==='') {
			 $Return['error'] = $this->lang->line('graphene_employee_error_to_date');
		} else if($frm_date > $to_date) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_frm_to_date');
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('employee_contract_info_added');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function e_contract_info() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('employee_contract_info_updated');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function leave_info() {
		...
		if($this->input->post('contract_id')==='') {
       		 $Return['error'] = $this->lang->line('graphene_employee_error_contract_f');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('employee_leave_info_added');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function e_leave_info() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('employee_leave_info_updated');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function shift_info() {
		...
		if($this->input->post('from_date')==='') {
			$Return['error'] = $this->lang->line('graphene_employee_error_frm_date');
			 ...
			 $Return['error'] = $this->lang->line('graphene_employee_error_shift_field');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('employee_shift_info_added');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function e_shift_info() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('employee_shift_info_updated');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function location_info() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_location_info_added');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function e_location_info() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_location_info_updated');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function change_password() {
		...
		if(trim($this->input->post('new_password'))==='') {
       		 $Return['error'] = $this->lang->line('graphene_employee_error_newpassword');
		} else if(strlen($this->input->post('new_password')) < 6) {
			$Return['error'] = $this->lang->line('graphene_employee_error_password_least');
		} else if(trim($this->input->post('new_password_confirm'))==='') {
			 $Return['error'] = $this->lang->line('graphene_employee_error_new_cpassword');
		} else if($this->input->post('new_password')!=$this->input->post('new_password_confirm')) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_old_new_cpassword');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_password_update');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function contacts()
     {
        ...
        foreach($contacts->result() as $r) {
			...
			if($r->is_primary==1){
				$primary = '<span class="tag tag-success">'.$this->lang->line('graphene_employee_primary').'</span>';
			 } else {
				 $primary = '';
			 }
			 if($r->is_dependent==2){
				$dependent = '<span class="tag tag-danger">'.$this->lang->line('graphene_employee_dependent').'</span>';
			 } else {
				 $dependent = '';
			 }
		
		$data[] = array(
			'<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_edit').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".edit-modal-data" data-field_id="'. $r->contact_id . '" data-field_type="contact"><i class="fa fa-pencil-square-o"></i></button></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_delete').'"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->contact_id . '" data-token_type="contact"><i class="fa fa-trash-o"></i></button></span>',
	...
	public function documents() {
        ...
        foreach($documents->result() as $r) {
			...
			$date_of_expiry = $this->Graphene_model->set_date_format($r->date_of_expiry);
			if($r->document_file!='' && $r->document_file!='no file') {
			 $functions = '<span data-toggle="tooltip" data-placement="top" title="Download"><a href="'.site_url().'download?type=document&filename='.$r->document_file.'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" title="'.$this->lang->line('graphene_download').'"><i class="fa fa-download"></i></button></a></span>';
			 } else {
				 $functions ='';
			 }
			 
			 if($r->is_alert==1){
			 	$alert = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_e_details_alert_notifyemail').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"><i class="fa fa-bell"></i></button></span>';
			 } else {
				 $alert = '';
			 }
		
		$data[] = array(
			$alert.$functions.'<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_edit').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".edit-modal-data" data-field_id="'. $r->document_id . '" data-field_type="document"><i class="fa fa-pencil-square-o"></i></button></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_delete').'"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->document_id . '" data-token_type="document"><i class="fa fa-trash-o"></i></button></span>',
	...
	public function immigration() {
		...
		$issue_date = $this->Graphene_model->set_date_format($r->issue_date);
		$expiry_date = $this->Graphene_model->set_date_format($r->expiry_date);
		$eligible_review_date = $this->Graphene_model->set_date_format($r->eligible_review_date);
		$d_type = $this->Employees_model->read_document_type_information($r->document_type_id);
	...
	public function qualification() {
		...
		$sdate = $this->Graphene_model->set_date_format($r->from_year);
		$edate = $this->Graphene_model->set_date_format($r->to_year);	
	...
	public function contract() {
        ...
        foreach($contract->result() as $r) {			
			...
			// date
			$duration = $this->Graphene_model->set_date_format($r->from_date).' '.$this->lang->line('dashboard_to').' '.$this->Graphene_model->set_date_format($r->to_date);
	...
	public function leave() {
        ...
        foreach($leave->result() as $r) {			
			...
			// contract duration
			$duration = $this->Graphene_model->set_date_format($contract[0]->from_date).' '.$this->lang->line('dashboard_to').' '.$this->Graphene_model->set_date_format($contract[0]->to_date);
	...
	public function update() {
		...
		if($this->input->post('warning_to')==='') {
       		 $Return['error'] = $this->lang->line('graphene_employee_error_warning');
		} else if($this->input->post('type')==='') {
			$Return['error'] = $this->lang->line('graphene_employee_error_warning_type');
		} else if($this->input->post('subject')==='') {
			 $Return['error'] = $this->lang->line('graphene_employee_error_subject');
		} else if(empty($this->input->post('warning_by'))) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_warning_by');
		} else if(empty($this->input->post('warning_date'))) {
			 $Return['error'] = $this->lang->line('graphene_employee_error_warning_date');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_employee_warning_updated');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function delete_contact() {
		...
		if($this->input->post('data')=='delete_record') {
			...
			if(isset($id)) {
				$Return['result'] = $this->lang->line('graphene_employee_contact_deleted');
			} else {
				$Return['error'] = $this->lang->line('graphene_error_msg');
			}
			$this->output($Return);
	...
	public function delete_qualification() {
		...
		if($this->input->post('data')=='delete_record') {
			...
			if(isset($id)) {
				$Return['result'] = $this->lang->line('employee_qualification_deleted');
			} else {
				$Return['error'] = $this->lang->line('graphene_error_msg');
			}
			$this->output($Return);
	...
	public function delete_work_experience() {
		...
		if($this->input->post('data')=='delete_record') {
			...
			if(isset($id)) {
				$Return['result'] = $this->lang->line('graphene_employee_work_deleted');
			} else {
				$Return['error'] = $this->lang->line('graphene_error_msg');
			}
			$this->output($Return);
	...
	public function delete_bank_account() {
		...
		if($this->input->post('data')=='delete_record') {
			...
			if(isset($id)) {
				$Return['result'] = $this->lang->line('employee_bankaccount_deleted');
			} else {
				$Return['error'] = $this->lang->line('graphene_error_msg');
			}
			$this->output($Return);
	...
	public function delete_contract() {
		...
		if($this->input->post('data')=='delete_record') {
			...
			if(isset($id)) {
				$Return['result'] = $this->lang->line('employee_contract_deleted');
			} else {
				$Return['error'] = $this->lang->line('graphene_error_msg');
			}
			$this->output($Return);
	...
	public function delete_leave() {
		...
		if($this->input->post('data')=='delete_record') {
			...
			if(isset($id)) {
				$Return['result'] = $this->lang->line('employee_leave_deleted');
			} else {
				$Return['error'] = $this->lang->line('graphene_error_msg');
			}
			$this->output($Return);
	...
	public function delete_shift() {
		...
		if($this->input->post('data')=='delete_record') {
			...
			if(isset($id)) {
				$Return['result'] = $this->lang->line('employee_shift_deleted');
			} else {
				$Return['error'] = $this->lang->line('graphene_error_msg');
			}
			$this->output($Return);
	...
	public function delete_location() {
		...
		if($this->input->post('data')=='delete_record') {
			...
			if(isset($id)) {
				$Return['result'] = $this->lang->line('graphene_employee_location_deleted');
			} else {
				$Return['error'] = $this->lang->line('graphene_error_msg');
			}
			$this->output($Return);
	...
	public function delete() {
		...
		if($this->input->post('is_ajax')=='2') {
			...
			if(isset($id)) {
				$Return['result'] = $this->lang->line('graphene_employee_current_deleted');
			} else {
				$Return['error'] = $this->lang->line('graphene_error_msg');
			}
			$this->output($Return);
