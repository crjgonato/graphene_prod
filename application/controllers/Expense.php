...
class Expense extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['breadcrumbs'] = $this->lang->line('expenses');
    ...
    public function expense_list()
     {
		...
		$data = array();
          ...
          foreach($expense->result() as $r) {
			  ...
			  // get date
			  $edate = $this->Graphene_model->set_date_format($r->purchase_date);
			  // get currency
			  $currency = $this->Graphene_model->currency_sign($r->amount);
			  // download
			  $download = '';
			  ...
			  if($r->billcopy_file!='' && $r->billcopy_file!='no file') {
			 	$download = '<span data-toggle="tooltip" data-placement="top" title="Download"><a href="download?type=expense&filename='.$r->billcopy_file.'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" title="'.$this->lang->line('graphene_download').'"><i class="fa fa-download"></i></button></a></span>';
			 }

               $data[] = array(
			   		'<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_edit').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-expense_id="'. $r->expense_id . '"><i class="fa fa-pencil-square-o"></i></button></span>'.$download.'<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_view').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-expense_id="'. $r->expense_id . '"><i class="fa fa-eye"></i></button></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('graphene_delete').'"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->expense_id . '"><i class="fa fa-trash-o"></i></button></span>',
                    $full_name,
                    $expense_type[0]->name,
                    $currency,
                    $edate,
	...
	public function add_expense() {
		...
		if($this->input->post('expense_type')==='') {
        	$Return['error'] = $this->lang->line('graphene_error_expense_type');
		} else if($this->input->post('purchase_date')==='') {
			$Return['error'] = $this->lang->line('graphene_error_purchase_date');
		} else if($this->input->post('amount')==='') {
			$Return['error'] = $this->lang->line('graphene_error_expense_amount');
		} else if($this->input->post('employee_id')==='') {
			$Return['error'] = $this->lang->line('graphene_error_employee_id');
		} 
		...
		/* Check if file uploaded..*/
			...
			if(is_uploaded_file($_FILES['bill_copy']['tmp_name'])) {
				...
				if(in_array($ext,$allowed)){
					...
					$Return['error'] = $this->lang->line('graphene_error_expense_file_type');
				}
			}
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_success_add_expense');
		} else {
	...
	public function update() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_success_update_expense');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function delete() {
		...
		if(isset($id)) {
			$Return['result'] = $this->lang->line('graphene_success_delete_expense');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
