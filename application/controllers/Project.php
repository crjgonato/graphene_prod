...
class Project extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['breadcrumbs'] = $this->lang->line('projects');
	 ...
	 public function detail()
     {
		...
		//$data['all_employees'] = $this->Graphene_model->all_employees();
		//$data['all_companies'] = $this->Graphene_model->get_companies();
		//$data['breadcrumbs'] = $this->lang->line('graphene_project_detail');
		$id = $this->uri->segment(3);
		$result = $this->Project_model->read_project_information($id);
		// get user > added by
		$user = $this->Graphene_model->read_user_info($result[0]->added_by);
		// user full name
		$full_name = $user[0]->first_name.' '.$user[0]->last_name;
		$data = array(
			'breadcrumbs' => $this->lang->line('graphene_project_detail'),
			...
			'all_companies' => $this->Graphene_model->get_companies()
			);
    ...
    public function project_list()
     {
		...
		$full_name = $user[0]->first_name.' '.$user[0]->last_name;
		// get date
		$pdate = '<i class="fa fa-calendar position-left"></i> '.$this->Graphene_model->set_date_format($r->end_date);
		...
		if($r->assigned_to == '') {
			...
			foreach(explode(',',$r->assigned_to) as $desig_id) {
				$assigned_to = $this->Graphene_model->read_user_info($desig_id);
	...
	public function add_project() {
		...
		if($this->input->post('title')==='') {
			...
			$Return['error'] = $this->lang->line('graphene_error_client_name');
		} else if($this->input->post('company_id')==='') {
			$Return['error'] = $this->lang->line('graphene_error_company');
			...
			$Return['error'] = $this->lang->line('graphene_error_start_end_date');
			 ...
			 $Return['error'] = $this->lang->line('graphene_error_project_manager');
		} else if($this->input->post('summary')==='') {
			$Return['error'] = $this->lang->line('graphene_error_summary');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_success_add_project');
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				$user_info = $this->Graphene_model->read_user_info($p_employee);				
					//get company info
				$cinfo = $this->Graphene_model->read_company_setting_info(1);
				//get email template
				$template = $this->Graphene_model->read_email_template(3);
				
				$subject = $template[0]->subject.' - '.$cinfo[0]->company_name;
				$logo = base_url().'uploads/logo/'.$cinfo[0]->logo;
				
				$p_date = $this->Graphene_model->set_date_format($start_date);
	...
	public function update() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('graphene_success_update_project');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
	...
	public function delete() {
		...
		if(isset($id)) {
			$Return['result'] = $this->lang->line('graphene_success_delete_project');
		} else {
			$Return['error'] = $this->lang->line('graphene_error_msg');
		}
		$this->output($Return);
