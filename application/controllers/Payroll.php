...
class Payroll extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function pdf_create() {
   	...
   	$pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
		...
		$user = $this->Graphene_model->read_user_info($payment[0]->employee_id);
		$_des_name = $this->Designation_model->read_designation_information($user[0]->designation_id);
		$department = $this->Department_model->read_department_information($user[0]->department_id);
		$location = $this->Graphene_model->read_location_info($department[0]->location_id);
		// company info
		$company = $this->Graphene_model->read_company_info($location[0]->company_id);
		...
		$country = $this->Graphene_model->read_country_info($company[0]->country);
		...
		if($payment[0]->house_rent_allowance!='' || $payment[0]->house_rent_allowance!=0){
			$hra = $this->Graphene_model->currency_sign($payment[0]->house_rent_allowance);
		} else { $hra = '0';}
		if($payment[0]->medical_allowance!='' || $payment[0]->medical_allowance!=0){
			$ma = $this->Graphene_model->currency_sign($payment[0]->medical_allowance);
		} else { $ma = '0';}
		if($payment[0]->travelling_allowance!='' || $payment[0]->travelling_allowance!=0){
			$ta = $this->Graphene_model->currency_sign($payment[0]->travelling_allowance);
		} else { $ta = '0';}
		if($payment[0]->dearness_allowance!='' || $payment[0]->dearness_allowance!=0){
			$da = $this->Graphene_model->currency_sign($payment[0]->dearness_allowance);
		} else { $da = '0';}
		
		// Deductions
		if($payment[0]->provident_fund!='' || $payment[0]->provident_fund!=0){
			$pf = $this->Graphene_model->currency_sign($payment[0]->provident_fund);
		} else { $pf = '0';}
		if($payment[0]->tax_deduction!='' || $payment[0]->tax_deduction!=0){
			$td = $this->Graphene_model->currency_sign($payment[0]->tax_deduction);
		} else { $td = '0';}
		if($payment[0]->security_deposit!='' || $payment[0]->security_deposit!=0){
			$sd = $this->Graphene_model->currency_sign($payment[0]->security_deposit);
		} else { $sd = '0';}
		...
		<table cellpadding="5" cellspacing="0" border="1">
			<tr style="background-color:#c4e5fd;">
			 ...
			 <tr>
				...
				<td colspan="2" align="right">'.$this->Graphene_model->currency_sign($payment[0]->basic_salary).'</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Gross Salary</td>
				<td align="right">'.$this->Graphene_model->currency_sign($payment[0]->gross_salary).'</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Total Allowance</td>
				<td align="right">'.$this->Graphene_model->currency_sign($payment[0]->total_allowances).'</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Total Deduction</td>
				<td align="right">'.$this->Graphene_model->currency_sign($payment[0]->total_deductions).'</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Net Salary</td>
				<td align="right">'.$this->Graphene_model->currency_sign($payment[0]->net_salary).'</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Paid Amount</td>
		...
		<table cellpadding="5" cellspacing="0" border="1">
			...
			<tr>
				...
				<td colspan="2" align="right">'.$this->Graphene_model->currency_sign($payment[0]->hourly_rate).'</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Gross Salary</td>
				<td align="right">'.$this->Graphene_model->currency_sign($payment[0]->payment_amount).'</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Net Salary</td>
	 ...
	 public function payslip()
     {
		...
		$user = $this->Graphene_model->read_user_info($result[0]->employee_id);
		// user full name
	 ...
	 public function reference() {
		$data['title'] = $this->Graphene_model->site_title();
		// $data['all_employees'] = $this->Graphene_model->all_employees();
		$data['breadcrumbs'] = 'Reference';
		$data['path_url'] = 'reference';
		$session = $this->session->userdata('username');
		$role_resources_ids = $this->Graphene_model->user_role_resource();
		if(in_array('42',$role_resources_ids)) {
			if(!empty($session)){
			$data['subview'] = $this->load->view("payroll/reference", $data, TRUE);
			$this->load->view('layout_main', $data); //page load
			} else {
				redirect('');
			}
	 ...
	 public function dforms() {
		$data['title'] = $this->Graphene_model->site_title();
		// $data['all_employees'] = $this->Graphene_model->all_employees();
		$data['breadcrumbs'] = 'Downloadable Forms';
		$data['path_url'] = 'dforms';
		$session = $this->session->userdata('username');
		$role_resources_ids = $this->Graphene_model->user_role_resource();
		if(in_array('42',$role_resources_ids)) {
			if(!empty($session)){
			$data['subview'] = $this->load->view("payroll/d-forms", $data, TRUE);
			$this->load->view('layout_main', $data); //page load
			} else {
				redirect('');
			}
  	...
  	public function template_list() {
          ...
          foreach($template->result() as $r) {
			  ...
			  // get basic salary
			  $sbs = $this->Graphene_model->currency_sign($r->basic_salary);
			  // get net salary
			  $sns = $this->Graphene_model->currency_sign($r->net_salary);
			  // get date > created at > and format
			  $cdate = $this->Graphene_model->set_date_format($r->created_at);
			  // total allowance
				// if($r->total_allowance == 0 || $r->total_allowance=='') {
				// 	$allowance = '-';
				// } else{
				// 	$allowance = $this->Graphene_model->currency_sign($r->total_allowance);
				// }
	 ...
	 public function payment_history_list()
     {
		...
		$data = array();
          ...
          foreach($history->result() as $r) {
			  ...
			  $p_amount = $this->Graphene_model->currency_sign($r->payment_amount);
	 ...
	 public function hourly_list() {
		...
		$data = array();
          ...
          foreach($hourly_wages->result() as $r) {
			  ...
			  $cdate = $this->Graphene_model->set_date_format($r->created_at);
			   // get hourly rate
			  $hourly_rate = $this->Graphene_model->currency_sign($r->hourly_rate);
	 ...
	 // gegenerate_payslip list table/ payment info
	 // hourly_list > templates
          ...
          foreach($payslip->result() as $r) {
			...
			if($r->monthly_grade_id =='' || $r->monthly_grade_id ==0) {
				...
				if($grade_template[0]->salary_grades){
					...
					$template = $grade_template[0]->salary_grades;
			...
			if($payment_check->num_rows() > 0){
				...
				$p_details = '<span data-toggle="tooltip" data-placement="top" title="View"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".detail_modal_data" data-employee_id="'. $r->user_id . '" data-pay_id="'. $make_payment[0]->make_payment_id . '"><i class="fa fa-arrow-circle-right"></i></button></span>';
					...
					if($net_salary > 0) {
					$functions = '<span data-toggle="tooltip" data-placement="top" title="Calculate Salary"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".'.$p_class.'" data-employee_id="'. $r->user_id . '" data-payment_date="'. $p_date . '">Create Salary</button></span>';
					} else {
					$functions = "<span class='text-danger' data-toggle='tooltip' data-placement='left' title='You cant create salary because net salary is 0'>0 Net Salary</span>";
					}
	...
	public function pay_monthly()
	{
        ...
        // get addd by > template
		$user = $this->Graphene_model->read_user_info($id);
	...
	public function template_read()
	{
       ...
       // $data['all_countries'] = $this->graphene_model->get_countries();
		...
		$data = array(
				...
				'basic_salary' => $result[0]->basic_salary,
				'overtime_rate' => $result[0]->overtime_rate,
				'ssspremium' => $result[0]->ssspremium,
				'philpremium' => $result[0]->philpremium,
				'hdmfpremium' => $result[0]->hdmfpremium,
				'sssloan' => $result[0]->sssloan,
				'hdmfloan' => $result[0]->hdmfloan,
				'wdholdingtax' => $result[0]->wdholdingtax,
				'latepenalty' => $result[0]->latepenalty,
				'uniforms' => $result[0]->uniforms,
				'drnumber' => $result[0]->drnumber,
	...
	public function add_template() {
		...
		'ssspremium' => $this->input->post('ssspremium'),
		'philpremium' => $this->input->post('philpremium'),
		'hdmfpremium' => $this->input->post('hdmfpremium'),
		'sssloan' => $this->input->post('sssloan'),
		'hdmfloan' => $this->input->post('hdmfloan'),
		'wdholdingtax' => $this->input->post('wdholdingtax'),
		'latepenalty' => $this->input->post('latepenalty'),
		'uniforms' => $this->input->post('uniforms'),
		'drnumber' => $this->input->post('drnumber'),
	...
	public function add_pay_monthly() {
		...
		if ($result == TRUE) {
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				//get email template
				$template = $this->Graphene_model->read_email_template(1);
				//get employee info
				$user_info = $this->Graphene_model->read_user_info($this->input->post('emp_id'));
