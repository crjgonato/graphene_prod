...
class job_post_model extends CI_Model {
	...
	public function get_jobs_candidates() {
	  return $this->db->get("job_applications");
	}
	
	// get all employee applied jobs
	public function get_employee_jobs_applied($id) {
		return $query = $this->db->query("SELECT * from job_applications where user_id = '".$id."'");
	}
	 // read job info
	...
	public function all_job_types() {
	  $query = $this->db->query("SELECT * from job_type");
  	  return $query->result();
	}
	...
	{
	  $query = $this->db->query("SELECT j.*, jap.* FROM jobs as j, job_applications as jap where j.job_id = jap.job_id group by j.job_id");
  	  return $query->result();
	}
	...
	public function all_interviews() {
	  return $this->db->get("job_interviews");
	}
