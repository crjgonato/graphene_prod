...
class transfers_model extends CI_Model {
	...
	public function get_transfers() {
	  return $this->db->get("employee_transfer");
	}
	
	public function get_employee_transfers($id) {
	 	return $query = $this->db->query("SELECT * from employee_transfer where employee_id = '".$id."'");
	}
	...
	public function delete_record($id){
		...
		$this->db->delete('employee_transfer');
	...
	public function update_record($data, $id){
		$this->db->where('transfer_id', $id);
		if( $this->db->update('employee_transfer',$data)) {
			return true;
		} else {
			return false;
		}		
