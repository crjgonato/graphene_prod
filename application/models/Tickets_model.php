...
class tickets_model extends CI_Model {
	...
	public function get_tickets() {
	  return $this->db->get("support_tickets");
	}
	...
	public function add_new_attachment($data){
		$this->db->insert('tickets_attachment', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	public function delete_comment_record($id){
		...
		$this->db->delete('tickets_comments');
	...
	public function delete_attachment_record($id){
		...
		$this->db->delete('tickets_attachment');
		
	}
	
	public function get_employee_tickets($id) {
	 	return $query = $this->db->query("SELECT * from support_tickets where employee_id = '".$id."'");
	}
	
	// Function to update record in table
	public function update_record($data, $id){
		$this->db->where('ticket_id', $id);
		if( $this->db->update('support_tickets',$data)) {
			return true;
		} else {
			return false;
		}		
	...
	public function get_attachments($id) {
		...
		$condition = "ticket_id =" . "'" . $id . "'";
		$this->db->select('*');
		$this->db->from('tickets_attachment');
		$this->db->where($condition);
		$this->db->limit(100);
		$query = $this->db->get();
